...
def post_human_message():
    HUMAN_MESSAGE = request.json['humanText']
    print(session)
    ID = session.get("cogniflow_id")
...
def initialize():
    #### 0) INITIALIZATION
    NUM_SENTENCES = int(request.json['numSentences'])
    PATH_TO_FILE = request.json['url']
    ...
    session["cogniflow_id"] = config.id
    print(session)
